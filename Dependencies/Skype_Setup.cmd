...
for %%a in (Dependencies\Downloads\Skype-%version%.exe) do set setup_file=%%a
for %%i in (Dependencies\Downloads\innounp*.rar) do set innounp=%%i
set a=%0
set b=%1
set c=%2
set d=%3
...
echo.
7z e %innounp% -o%app_folder%\%temporary%>nul 2>&1
%app_folder%\%temporary%\innounp.exe %setup_file% -x -d%app_folder%>nul 2>&1
rd %app_folder%>nul 2>&1
if not exist %app_folder% goto error
cd %app_folder%
call :ren {app} App
call :del install_script.iss
call :rd %temporary%
cd..
