...
set version=*
for %%a in (Dependencies\Downloads\ccsetup%version%.exe) do set setup_file=%%a
set a=%0
set b=%1
set c=%2
set d=%3
...
echo.
7z x %setup_file% -o%app_folder%\App CCleaner*.exe>nul 2>&1
rd %app_folder%>nul 2>&1
if not exist %app_folder% goto error
type nul>%app_folder%\App\ccleaner.ini
