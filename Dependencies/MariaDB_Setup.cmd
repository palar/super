...
set app_folder=%server_folder%\App
set version=*
set bit=x64
if not defined ProgramFiles(x86) set bit=32
for %%a in (Dependencies\Downloads\mariadb-%version%-win%bit%.msi) do set setup_file=%%a
set a=%0
set b=%1
set c=%2
set d=%3
...
echo.
msiexec /a %setup_file% TARGETDIR=%apps_dir%\%app_folder% /quiet
rd %app_folder%>nul 2>&1
if not exist %app_folder% goto error
pushd %app_folder%
call :del %setup_file:Dependencies\Downloads\=%
call :rd MariaDBShared
for /d %%a in ("MariaDB *") do set temporary="%%a"
call :ren %temporary% %app_name%
popd
...
:uninstall
call :rd %app_folder%\%app_name%
rd %app_folder%>nul 2>&1
rd %server_folder%>nul 2>&1
goto end
