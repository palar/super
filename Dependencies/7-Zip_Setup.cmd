...
for %%a in (Dependencies\Downloads\7z%version%%bit%*) do (
	set temporary=%%~na
	set setup_file=%%a
)
set a=%0
...
if not "%setup_file:.msi=%" == "%setup_file%" goto install-msi
call :rd %app_folder%\%temporary%
echo Installing %app_name%...
echo.
7z x %setup_file% -o%app_folder%\%temporary%>nul 2>&1
rd %app_folder%>nul 2>&1
if not exist %app_folder% goto error
cd %app_folder%
call :rd App
call :ren %temporary% App
call :del App\Uninstall.exe
cd..
...
robocopy Files\7-Zip App /e /move>nul 2>&1
call :rd Files
cd..
echo %app_name% has been successfully installed.
goto end
