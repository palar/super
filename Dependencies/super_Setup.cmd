...
for %%a in (Dependencies\Downloads\%app_folder%.*) do (
	set temporary=%%~na_Temp
	set setup_file=%%a
)
set a=%0
set b=%1
...
echo.
7z x %setup_file% -o%temporary% -aoa>nul 2>&1
rd %temporary%>nul 2>&1
if not exist %temporary% goto error
robocopy "%temporary%" . /move /xf %app_name%.txt>nul 2>&1
robocopy "%temporary%\7-Zip" 7-Zip /move /mir>nul 2>&1
robocopy "%temporary%\Dependencies" Dependencies /move>nul 2>&1
robocopy "%temporary%\html" html /move /mir>nul 2>&1
robocopy "%temporary%\ngrok" ngrok /move /mir>nul 2>&1
robocopy "%temporary%\Settings" Settings /move /mir>nul 2>&1
call :del %app_name%.txt
call :rd %temporary%
