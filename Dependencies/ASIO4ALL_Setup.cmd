...
for %%a in (Dependencies\Downloads\ASIO4ALL_%version%_English.exe) do set setup_file=%%a
set a=%0
set b=%1
set c=%2
set d=%3
...
echo.
7z x %setup_file% -o%app_folder%\App *.dll -aos>nul 2>&1
rd %app_folder%>nul 2>&1
if not exist %app_folder% goto error
regsvr32 /s %app_folder%\App\asio4all%bit%.dll
...
:uninstall
regsvr32 /u /s %app_folder%\App\asio4all%bit%.dll
call :rd %app_folder%\App
rd %app_folder%>nul 2>&1
reg delete "HKEY_CURRENT_USER\Software\ASIO4ALL v2 by Wuschel" /f>nul 2>&1
reg delete HKEY_LOCAL_MACHINE\SOFTWARE\ASIO /f>nul 2>&1
goto end
