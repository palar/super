...
for /f "tokens=2" %%i in ("%app_name%") do set app_folder=%%i
set version=*
set bit=64
if not defined ProgramFiles(x86) set bit=86
for %%a in (Dependencies\Downloads\MicrosoftEdge_X%bit%_%version%.exe) do set setup_file=%%a
set a=%0
set b=%1
set c=%2
set d=%3
...
echo.
7z e %setup_file% -o%app_folder% MSEDGE.7z>nul 2>&1
rd %app_folder%>nul 2>&1
if not exist %app_folder% goto error
cd %app_folder%
7z x MSEDGE.7z>nul 2>&1
call :ren Chrome-bin App
call :del MSEDGE.7z
cd..
...
call :rd %app_folder%\Chrome-bin
call :del %app_folder%\MSEDGE.7z
rd %app_folder%>nul 2>&1
goto end
