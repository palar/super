...
set version=*
set bit=x86_64
if not defined ProgramFiles(x86) set bit=i686
for %%a in (Dependencies\Downloads\mpv-%version%-%bit%.7z) do set setup_file=%%a
for %%a in (Dependencies\Downloads\mpv-%bit%-%version%.7z) do set setup_file=%%a
set a=%0
set b=%1
set c=%2
set d=%3
...
echo.
7z x %setup_file% -o%app_folder%\App>nul 2>&1
rd %app_folder%>nul 2>&1
if not exist %app_folder% goto error
call :md %app_folder%\App\portable_config
